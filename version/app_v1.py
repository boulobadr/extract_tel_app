...
# --- Interface Streamlit ---
st.sidebar.image("https://upload.wikimedia.org/wikipedia/commons/thumb/3/3c/Phone_font_awesome.svg/512px-Phone_font_awesome.svg.png", width=100)
st.sidebar.title("📞 Analyse Téléphonique")
st.sidebar.markdown("Chargez un fichier CSV contenant une colonne **Erreur** avec des numéros à analyser.")
...
if uploaded_file:
    ...
    if "Erreur" not in df.columns:
        ...
        with col2:
            ...
            if country_counts:
                fig, ax = plt.subplots()
                ax.pie(country_counts.values(), labels=country_counts.keys(), autopct='%1.1f%%', startangle=90)
                ax.axis('equal')
                st.pyplot(fig)
            else:
                st.info("Aucun pays valide détecté pour générer un graphique.")
